...
export default function PaymentFailed() {
  ...
  useEffect(() => {
    ...
    const transactionId = urlParams.get('txn_response_code') || urlParams.get('id');
  ...
  const handleContactSupport = () => {
    ...
    console.log('Contact support for failed transaction:', errorData?.transactionId);
  ...
  const getErrorDescription = (errorCode: string | null, errorMessage: string | null) => {
    if (errorMessage) return errorMessage;
    
    switch (errorCode) {
      case 'DECLINED':
        return t('payment', 'errorDeclined') || 'Your payment was declined by the bank. Please try a different payment method.';
      case 'INSUFFICIENT_FUNDS':
        return t('payment', 'errorInsufficient') || 'Your account has insufficient funds. Please check your balance and try again.';
      case 'EXPIRED_CARD':
        return t('payment', 'errorExpired') || 'The payment card has expired. Please use a valid card.';
      case 'INVALID_CARD':
        return t('payment', 'errorInvalid') || 'The card information is invalid. Please check your card details.';
      case 'TIMEOUT':
        return t('payment', 'errorTimeout') || 'The payment request timed out. Please try again.';
      default:
        return t('payment', 'failedSubtitle') || 'An error occurred while processing your payment. Please try again.';
    }
  };
        ...
        <Card className="shadow-2xl border-0">
          <CardHeader className="text-center pb-6">
            ...
            <p className="text-gray-600 mt-2">
              {getErrorDescription(errorData?.errorCode, errorData?.errorMessage)}
            </p>
          </CardHeader>
          ...
          <CardContent className="space-y-6">
                ...
                <AlertDescription>
                  <div className="mt-2 space-y-1">
                    <div className="flex justify-between">
                      <span className="font-medium">{t('payment', 'errorCode')}:</span>
                    ...
                    {errorData.reason && (
                      <div className="flex justify-between">
                        <span className="font-medium">{t('payment', 'reason')}:</span>
            ...
            {errorData && (
              <div className="bg-gray-50 rounded-lg p-4 space-y-3">
                <h3 className="font-medium text-gray-900 mb-3">{t('payment', 'transactionDetails')}</h3>
